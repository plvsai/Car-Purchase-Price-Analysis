d3.csv("data/car_purchasing.csv").then((csvData) => {
  ...
  const variableSelect = d3.select("#variableSelect").on("change", (event) => {
    ...
    d3.select("#variableDistributionTitle").text(variable.name);

    variableDistributionChart.updateVariable(variable);
  });
  ...
  const priceRangeInput = d3.select("#priceRangeInput").on("change", () => {
    ...
    variableDistributionChart.updateData(filteredData);
  });
  doubleRangeInput(
    priceRangeInput,
    priceRangeOutput,
  ...
  // Color change
  let colorIndex = 2;
  const colorButton = d3.select("#colorButton").on("click", () => {
    colorIndex = (colorIndex + 1) % 8;
    variableDistributionChart.updateColor(d3.schemeSet2[colorIndex]);
  });
  ...
  const variableDistributionChart = histogramChart(
    d3.select("#variableDistributionChart"),
    filteredData,
    variable,
    d3.schemeSet2[colorIndex]
  );
});
